...
struct ContentView: View {
    ...
    @State private var editingEntry: ClassEntry?
    ...
    @ViewBuilder
    private func row(_ entry: ClassEntry) -> some View {
        HStack(alignment: .top, spacing: 12) {
            VStack(alignment: .leading, spacing: 4) {
                Text(entry.classtype)
                    .font(AppTheme.headlineFont)
                    .foregroundStyle(AppTheme.ink)
                Text("Duration (min): \(entry.value1)  \u{00B7}  Resistance (1-10): \(entry.value2)")
                    .font(.caption)
...
struct EntrySheet: View {
    ...
    @State private var option: String = SpinlogOptions.all.first ?? ""
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Class Type") {
                    Picker("Class Type", selection: $option) {
                        ForEach(SpinlogOptions.all, id: \.self) { t in
                            Text(t).tag(t)
                        }
                    }
                    .pickerStyle(.menu)
            ...
            .navigationTitle(entry == nil ? "New Class" : "Edit Class")
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        if var existing = entry {
                            existing.classtype = option
                            existing.value1 = Int(value1)
                            existing.value2 = Int(value2)
                            existing.note = note
                            store.update(existing)
                        } else {
                            store.add(classtype: option, value1: Int(value1), value2: Int(value2), note: note)
                        }
                        dismiss()
                    }
